...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-03T07:32:51+00:00",
...
**هوش مصنوعی** یک رشته علمی هیجان‌انگیز است که مطالعه می‌کند چگونه می‌توان کامپیوترها را به رفتار هوشمندانه واداشت، به عنوان مثال انجام کارهایی که انسان‌ها در آن‌ها مهارت دارند.

در ابتدا، کامپیوترها توسط [چارلز ببیج](https://en.wikipedia.org/wiki/Charles_Babbage) برای کار با اعداد و اجرای یک فرآیند مشخص - یک الگوریتم - اختراع شدند. کامپیوترهای مدرن، با وجود پیشرفت‌های چشمگیر نسبت به مدل اولیه‌ای که در قرن نوزدهم پیشنهاد شد، همچنان از همان ایده محاسبات کنترل‌شده پیروی می‌کنند. بنابراین، امکان برنامه‌ریزی کامپیوتر برای انجام کاری وجود دارد، اگر دقیقاً بدانیم که چه مراحل متوالی برای رسیدن به هدف لازم است.
...
> ✅ تعیین سن یک فرد از روی عکس او وظیفه‌ای است که نمی‌توان آن را به طور صریح برنامه‌ریزی کرد، زیرا نمی‌دانیم چگونه وقتی این کار را انجام می‌دهیم، عددی در ذهنمان ظاهر می‌شود.

---

با این حال، برخی وظایف وجود دارند که ما نمی‌دانیم چگونه به طور صریح آن‌ها را حل کنیم. به عنوان مثال، تعیین سن یک فرد از روی عکس او. ما به نوعی یاد می‌گیریم این کار را انجام دهیم، زیرا نمونه‌های زیادی از افراد در سنین مختلف دیده‌ایم، اما نمی‌توانیم به طور صریح توضیح دهیم چگونه این کار را انجام می‌دهیم و نمی‌توانیم کامپیوتر را برای انجام آن برنامه‌ریزی کنیم. این دقیقاً نوع وظایفی است که برای **هوش مصنوعی** (AI) جالب است.

✅ به برخی وظایفی فکر کنید که می‌توانید آن‌ها را به کامپیوتر واگذار کنید و از هوش مصنوعی بهره‌مند شوید. به حوزه‌های مالی، پزشکی و هنر فکر کنید - این حوزه‌ها امروز چگونه از هوش مصنوعی بهره می‌برند؟
...
این سیستم‌های هوش مصنوعی به طور کلی هوشمند نیستند؛ آن‌ها در انجام یک وظیفه از پیش تعریف‌شده عالی عمل می‌کنند اما فاقد درک واقعی یا آگاهی هستند.|این سیستم‌های هوش مصنوعی توانایی انجام هر وظیفه فکری که انسان قادر به انجام آن است، تطبیق با حوزه‌های مختلف و داشتن نوعی آگاهی یا خودآگاهی را دارند.
نمونه‌هایی از هوش مصنوعی ضعیف شامل دستیارهای مجازی مانند Siri یا Alexa، الگوریتم‌های توصیه‌گر مورد استفاده در سرویس‌های پخش و چت‌بات‌هایی است که برای وظایف خاص خدمات مشتری طراحی شده‌اند.|دستیابی به هوش مصنوعی قوی یک هدف بلندمدت در تحقیقات هوش مصنوعی است و نیازمند توسعه سیستم‌های هوش مصنوعی است که بتوانند استدلال کنند، یاد بگیرند، درک کنند و در طیف گسترده‌ای از وظایف و زمینه‌ها تطبیق یابند.
...
برای دیدن ابهام اصطلاح *هوش*، سعی کنید به این سوال پاسخ دهید: "آیا یک گربه هوشمند است؟". افراد مختلف تمایل دارند پاسخ‌های متفاوتی به این سوال بدهند، زیرا هیچ آزمون جهانی پذیرفته‌شده‌ای برای اثبات صحت این ادعا وجود ندارد. و اگر فکر می‌کنید وجود دارد - سعی کنید گربه خود را از یک آزمون IQ عبور دهید...
...
> یک چت‌بات به نام [Eugene Goostman](https://en.wikipedia.org/wiki/Eugene_Goostman)، که در سن‌پترزبورگ توسعه داده شده بود، در سال 2014 با استفاده از یک ترفند شخصیتی هوشمندانه نزدیک به گذراندن آزمون تورینگ رسید. این چت‌بات از ابتدا اعلام کرد که یک پسر 13 ساله اوکراینی است، که کمبود دانش و برخی تناقضات در متن را توضیح می‌داد. این چت‌بات 30٪ از داوران را پس از یک گفتگوی 5 دقیقه‌ای متقاعد کرد که انسان است، معیاری که تورینگ معتقد بود یک ماشین تا سال 2000 قادر به گذراندن آن خواهد بود. با این حال، باید درک کرد که این نشان نمی‌دهد که ما یک سیستم هوشمند ایجاد کرده‌ایم یا اینکه یک سیستم کامپیوتری بازپرس انسانی را فریب داده است - سیستم انسان‌ها را فریب نداد، بلکه سازندگان چت‌بات این کار را کردند!
...
* یک رویکرد **ظهوریافته**، **هم‌افزایی** یا **چندعاملی** بر اساس این واقعیت است که رفتار هوشمند پیچیده می‌تواند از تعامل تعداد زیادی عامل ساده به دست آید. طبق [سایبرنتیک تکاملی](https://en.wikipedia.org/wiki/Global_brain#Evolutionary_cybernetics)، هوش می‌تواند از رفتار واکنشی ساده‌تر در فرآیند **انتقال متاسیستم** *ظهور* کند.
...
در یک **رویکرد بالا به پایین**، ما سعی می‌کنیم استدلال خود را مدل‌سازی کنیم. از آنجا که می‌توانیم افکار خود را هنگام استدلال دنبال کنیم، می‌توانیم سعی کنیم این فرآیند را رسمی کنیم و آن را در داخل کامپیوتر برنامه‌ریزی کنیم. این به عنوان **استدلال نمادین** شناخته می‌شود.
...
این رویکرد به شدت به **نمایش دانش** و **استدلال** متکی است. استخراج دانش از یک متخصص انسانی ممکن است دشوارترین بخش باشد، زیرا یک پزشک در بسیاری از موارد دقیقاً نمی‌داند چرا به یک تشخیص خاص می‌رسد. گاهی اوقات راه‌حل به سادگی بدون فکر صریح در ذهن او ظاهر می‌شود. برخی وظایف، مانند تعیین سن یک فرد از یک عکس، اصلاً نمی‌توانند به دستکاری دانش کاهش یابند.

### رویکرد پایین به بالا

به طور متناوب، می‌توانیم سعی کنیم ساده‌ترین عناصر داخل مغز خود - یک نورون - را مدل‌سازی کنیم. می‌توانیم یک **شبکه عصبی مصنوعی** در داخل کامپیوتر بسازیم و سپس سعی کنیم آن را با دادن مثال‌ها آموزش دهیم تا مشکلات را حل کند. این فرآیند مشابه نحوه یادگیری یک کودک تازه متولد شده درباره محیط اطراف خود از طریق مشاهده است.
...
* در آینده، ممکن است انتظار داشته باشیم یک مدل کاملاً مبتنی بر شبکه عصبی بتواند به طور کامل گفت‌وگو را مدیریت کند. خانواده اخیر شبکه‌های عصبی GPT و [Turing-NLG](https://www.microsoft.com/research/blog/turing-nlg-a-17-billion-parameter-language-model-by-microsoft) موفقیت‌های بزرگی در این زمینه نشان داده‌اند.
...
در چند سال گذشته شاهد موفقیت‌های چشمگیری با مدل‌های زبانی بزرگ مانند BERT و GPT-3 بوده‌ایم. این موفقیت‌ها عمدتاً به دلیل وجود حجم زیادی از داده‌های متنی عمومی است که به ما امکان می‌دهد مدل‌هایی را آموزش دهیم تا ساختار و معنای متن‌ها را درک کنند، آن‌ها را بر روی مجموعه‌های عمومی متن پیش‌آموزش دهیم و سپس این مدل‌ها را برای وظایف خاص‌تر تخصصی کنیم. در ادامه این دوره، درباره [پردازش زبان طبیعی](../5-NLP/README.md) بیشتر خواهیم آموخت.
...
تاریخچه هوش مصنوعی و یادگیری ماشین را با مطالعه [این درس](https://github.com/microsoft/ML-For-Beginners/tree/main/1-Introduction/2-history-of-ML) مرور کنید. یکی از عناصر موجود در یادداشت تصویری بالای آن درس یا این درس را انتخاب کنید و درباره آن به طور عمیق تحقیق کنید تا زمینه فرهنگی مؤثر بر تکامل آن را درک کنید.
